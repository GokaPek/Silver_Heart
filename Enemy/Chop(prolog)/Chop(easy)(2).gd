...
func ctrl():
	if(global_position.x > get_node("../../StaticBody2D/CollisionShape2D4").global_position.x):
		return 0
	else:
		return plus_x.randi_range(-400,(get_node("../../StaticBody2D/CollisionShape2D4").global_position.x - targetBody.global_position.x))
