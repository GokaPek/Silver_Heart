...
func _physics_process(delta):
	Y_sort_who_near()
	if get_fatal_damage == true:
		in_bonk(delta, pos_before_fatal_bonk, self_dir_of_bonk)
	if $go.time_left != 0:
		if can_walk:
			look()
			walk()
...
func can_attack():
	if (global_position.distance_to(the_nearest().global_position) < 60) and HP > 5:
		can_walk = false
		if can_beat and HP > 5:
...
func in_bonk(var delta, var pos_B, var dir_bonk):
	pos_B.x = pos_B.x + (30 * dir_bonk)
	global_position = global_position.linear_interpolate(pos_B, delta * 2)



func get_damage(var pos_from):      #позиция того, кто ударил
	can_walk = false
	damage_now = true
	can_beat = false
	HP -= 5
...
func _on_wait_timeout():
	...
	plus_x_var = plus_x.randi_range(-130,330)
	plus_y_var = plus_y.randi_range(-130,330)
